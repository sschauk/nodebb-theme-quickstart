...
<div component="post/footer" class="post-footer border-bottom pb-2">
	...
	<div component="post/actions" class="d-flex justify-content-end gap-1 post-tools">
		...
		{{{ if privileges.isAdminOrMod }}}
			<!-- If the topic is not pinned, show the Pin option -->
			<a component="topic/pin" href="#" class="btn-ghost-sm d-flex align-items-center gap-1" role="menuitem" {{{ if topic.pinned }}} hidden{{{ end }}}>
				<i class="fa fa-fw fa-thumb-tack text-primary"></i> [[topic:thread-tools.pin]]
			</a>

			<!-- If the topic is pinned, show the Unpin option with a rotated icon -->
			<a component="topic/unpin" href="#" class="btn-ghost-sm d-flex align-items-center gap-1" role="menuitem" {{{ if !topic.pinned }}} hidden{{{ end }}}>
